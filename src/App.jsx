...
function App() {
  ...
  if (selectedHymn) return <main className="app-shell lyric-page"><button className="back-button" onClick={() => setSelectedHymn(null)}>← Volver al índice</button><h1>{selectedHymn.title}</h1><article className={`lyrics lyrics-${selectedHymn.category || 'adoracion'}`}>{selectedHymn.lyrics || selectedHymn.source ? selectedHymn.lyrics || new DOMParser().parseFromString(selectedHymn.source, 'text/html').querySelector('.letra')?.textContent.trim() || 'Esta letra todavía no tiene contenido.' : 'Esta letra todavía no tiene contenido.'}</article></main>;

  return <main className="app-shell"><header className="hero"><div className="hero-topline"><button className="brand" onClick={goHome}>PUERTA DEL CIELO</button><button className="admin-link" onClick={() => setShowAdmin(true)}>Panel administrativo</button></div><div className="hero-intro"><p className="eyebrow">HIMNARIO DIGITAL</p><h1>Una voz, muchas canciones.</h1><p className="hero-copy">Encuentra la alabanza que la iglesia cantará hoy.</p></div></header><section className="catalog"><div className="section-heading"><div><p className="eyebrow dark">CATÁLOGO</p><h2>{category ? category.name : 'Selecciona una sección'}</h2></div>{category && <button className="text-button" onClick={goHome}>Volver al inicio</button>}</div>{!category ? <div className="category-grid">{categories.map((item, index) => <button className="category-card" key={item.id} onClick={() => setCategoryId(item.id)}><span className="category-number">0{index + 1}</span><strong>{item.name}</strong><span>{item.description}</span><small>{item.id === 'servicio' ? `${cloudService.length || item.hymns.length} pasos programados` : `${cloudHymns.length ? cloudHymns.filter((hymn) => hymn.category === item.id).length : item.hymns.length} alabanzas`} <span aria-hidden="true">→</span></small></button>)}</div> : <><input className="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={isServiceProgram ? 'Buscar un paso...' : 'Buscar una alabanza...'} aria-label="Buscar" />{orderedItems.length ? <div className={isServiceProgram ? 'service-list' : 'hymn-list'}>{orderedItems.map((hymn, index) => <button className={isServiceProgram ? 'service-row' : 'hymn-row'} key={hymn.id} onClick={() => !isServiceProgram && setSelectedHymn(hymn)}><span>{String(index + 1).padStart(2, '0')}</span><strong>{isServiceProgram ? hymn.description : hymn.title}</strong>{!isServiceProgram && <b>→</b>}</button>)}</div> : <div className="empty-state">Aún no hay alabanzas cargadas en esta sección.</div>}</>}</section></main>;
}
...
function AdminPanel({ onClose }) {
  ...
  const saveStep = async (event) => { event.preventDefault(); const date = new Date().toISOString().slice(0, 10); const { data: last } = await supabase.from('service_program').select('step_number').eq('service_date', date).order('step_number', { ascending: false }).limit(1).maybeSingle(); const { data, error } = await supabase.from('service_program').insert({ service_date: date, step_number: (last?.step_number || 0) + 1, description: step }).select().single(); setProgramMessage(error?.message || `Paso guardado: ${data.description}`); if (!error) { setStep(''); setSteps((current) => [...current, data]); } };
  const deleteStep = async (item) => { const { error } = await supabase.from('service_program').delete().eq('id', item.id); if (!error) setSteps((current) => current.filter((saved) => saved.id !== item.id)); setProgramMessage(error?.message || 'Paso borrado.'); };
