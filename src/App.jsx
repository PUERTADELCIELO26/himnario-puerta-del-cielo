...
function AdminPanel({ onClose }) {
  ...
  useEffect(() => {
    ...
    supabase.from('service_program').select('*').eq('service_date', today).order('step_number').then(({ data }) => setSteps(data?.length ? data : (categories.find((item) => item.id === 'servicio')?.hymns || []).map((item, index) => ({ ...item, description: item.description || item.title, step_number: index + 1, localDefault: true }))));
  }, []);
  ...
  const deleteStep = async (item) => { if (item.localDefault) { setSteps((current) => current.filter((saved) => saved.id !== item.id)); setProgramMessage('Paso predeterminado quitado de esta vista.'); return; } const { error } = await supabase.from('service_program').delete().eq('id', item.id); if (!error) setSteps((current) => current.filter((saved) => saved.id !== item.id)); setProgramMessage(error?.message || 'Paso borrado.'); };
