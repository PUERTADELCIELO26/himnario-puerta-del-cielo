...
function getServiceStepOverrides() {
  try {
    return JSON.parse(localStorage.getItem(SERVICE_STEP_OVERRIDES_KEY) || '{}');
  } catch {
    return {};
  }
}
...
function App() {
  ...
  const serviceItems = (cloudService.length
    ? cloudService
    : (category?.hymns || []).map((item) => ({ ...item, localDefault: true })))
    .filter((item) => !item.localDefault || !getHiddenServiceSteps().includes(item.id))
    .map((item, index) => ({ ...item, description: getServiceStepOverrides()[item.id] || item.description || item.title || '', step_number: item.step_number || index + 1 }));
...
function AdminPanel({ onClose }) {
  ...
  const saveStep = async (event) => { event.preventDefault(); const description = step.trim(); let data; let error = null; if (editingStep?.localDefault) { const overrides = { ...getServiceStepOverrides(), [editingStep.id]: description }; localStorage.setItem(SERVICE_STEP_OVERRIDES_KEY, JSON.stringify(overrides)); data = { ...editingStep, description }; } else if (editingStep) { ({ data, error } = await supabase.from('service_program').update({ description }).eq('id', editingStep.id).select().single()); } else { const date = new Date().toISOString().slice(0, 10); const { data: last } = await supabase.from('service_program').select('step_number').eq('service_date', date).order('step_number', { ascending: false }).limit(1).maybeSingle(); ({ data, error } = await supabase.from('service_program').insert({ service_date: date, step_number: (last?.step_number || 0) + 1, description }).select().single()); } setProgramMessage(error?.message || (data ? `Paso ${editingStep ? 'actualizado' : 'guardado'}: ${data.description}` : `No se pudo ${editingStep ? 'actualizar' : 'guardar'} el paso.`)); if (!error) { setStep(''); setEditingStep(null); setSteps((current) => editingStep ? current.map((item) => item.id === data.id ? data : item) : [...current.filter((item) => !item.localDefault), data]); } };
  const editStep = (item) => { setEditingStep(item); setStep(item.description); };
  const cancelStepEdit = () => { setEditingStep(null); setStep(''); };
  ...
  return <main className="admin-page"><button className="back-button" onClick={onClose}>← Ver página</button><p className="eyebrow dark">{isEditor ? 'ACCESO DE EDITOR' : 'ACCESO ADMINISTRATIVO'}</p><h1>{isEditor ? 'Editor de alabanzas' : 'Panel administrativo'}</h1><div className="admin-tabs"><button onClick={() => setShowCatalog(!showCatalog)}>{showCatalog ? 'Ocultar catálogo' : 'Editar catálogo'}</button><button onClick={() => supabase.auth.signOut()}>Cerrar sesión</button></div>{!isEditor && <><h2>Nueva alabanza</h2><HymnForm hymn={hymn} setHymn={setHymn} saveHymn={saveHymn} message={hymnMessage} editing={editingHymn} cancel={resetEditor} /></>}{showCatalog && <><h2>Catálogo</h2><div className="service-list">{hymns.map((item) => <div className="service-row" key={item.id}><span>{item.number}</span><strong>{item.title}</strong><div className="row-actions"><button className="edit-button" type="button" onClick={() => editHymn(item)}>Editar</button>{!isEditor && <button className="delete-button" type="button" onClick={() => deleteHymn(item)}>Borrar</button>}</div>{isEditor && editingHymn?.id === item.id && <div className="inline-editor"><HymnForm hymn={hymn} setHymn={setHymn} saveHymn={saveHymn} message={hymnMessage} editing={editingHymn} cancel={resetEditor} /></div>}</div>)}</div></>}{!isEditor && <><h2>Programa de hoy</h2><form className="admin-form" onSubmit={saveStep}><input placeholder="Ej. Cantar alabanzas de avivamiento" value={step} onChange={(event) => setStep(event.target.value)} required /><div className="form-action"><button>{editingStep ? 'Actualizar paso' : 'Agregar paso'}</button>{editingStep && <button type="button" className="secondary-button" onClick={cancelStepEdit}>Cancelar</button>}{programMessage && <span className="form-message">{programMessage}</span>}</div></form><div className="service-list">{steps.map((item) => <div className="service-row" key={item.id}><span>{String(item.step_number).padStart(2, '0')}</span><strong>{item.description}</strong><div className="row-actions"><button className="edit-button" type="button" onClick={() => editStep(item)}>Editar</button><button className="delete-button" type="button" onClick={() => deleteStep(item)}>Borrar</button></div></div>)}</div></>}</main>;
}

function HymnForm({ hymn, setHymn, saveHymn, message, editing, cancel }) {
  return <form className="admin-form" onSubmit={saveHymn}><input placeholder="Título" value={hymn.title} onChange={(event) => setHymn({ ...hymn, title: event.target.value })} required /><label className="field-label" htmlFor="hymn-category">{editing ? 'Mover a sección' : 'Sección'}</label><select id="hymn-category" value={hymn.category} onChange={(event) => setHymn({ ...hymn, category: event.target.value })}><option value="adoracion">Adoración</option><option value="avivamiento">Avivamiento</option><option value="ninos">Alabanzas de niños</option></select><textarea placeholder="Letra" rows="12" value={hymn.lyrics} onChange={(event) => setHymn({ ...hymn, lyrics: event.target.value })} /><div className="form-action"><button>{editing ? 'Actualizar y mover alabanza' : 'Guardar alabanza'}</button>{editing && <button type="button" className="secondary-button" onClick={cancel}>Cancelar</button>}{message && <span className="form-message">{message}</span>}</div></form>;
}
